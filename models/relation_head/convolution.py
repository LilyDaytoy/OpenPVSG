...
class HandcraftedFilter(nn.Module):
    def __init__(self, feat_dim, num_relations):
        ...
        self.filter_weights = torch.tensor([1/4, 1/2, 1, 1/2, 1/4], dtype=torch.float32)
        self.expanded_filter_weights = self.filter_weights.view(1, 1, -1).repeat(feat_dim, 1, 1)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        filtered_x = F.conv1d(x, self.expanded_filter_weights.to(x.device), padding=2, groups=feat_dim)
...
class Learnable1DConv(nn.Module):
    def __init__(self, input_dim, num_relations, kernel_size=5, num_layers=1):
        ...
        for _ in range(num_layers):
            layers.append(nn.Conv1d(input_dim, input_dim, kernel_size, padding=kernel_size//2))

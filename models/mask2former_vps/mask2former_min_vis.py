...
class Mask2FormerVideoCustomMinVIS(SingleStageDetector):
    ...
    def simple_test(self, img, img_metas, ref_img, ref_img_metas, **kwargs):
        ...
        # (b, t, 3, h, w)
        ref_video = ref_img.reshape((bs * num_frame, three, h, w))
        # tuple of len 4 : ( (t, c, h, w),(t, c, h, w),(t, c, h, w),(t, c, h, w))
        video_x = self.extract_feat(ref_video)
        # origin clip level inference : cls: (b, n, c), mask_pred_results:  b,t,n,h,w
        # mini_vis input : cls (b, t, n, c), mask_pred_results: (b, t, n, h, w),query_fea: (b, t, n, c)
        pred_logits = []
        mask_pred_list = []
        query_pred_list = []
        for i in range(video_x[0].size(0)):
            ...
            mask_cls_results, mask_pred_results, query_fea = self.panoptic_head.simple_test_with_query(

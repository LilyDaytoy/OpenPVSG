...
class LoadVideo:  # for inference
    def __init__(self, path, img_size=(1088, 608)):
        ...
        self.w, self.h = self.get_size(self.vw, self.vh, self.width,
                                       self.height)
...
class LoadImagesAndObs:
    def __init__(self, path, opt):
        ...
        self.label_files = [
            x.replace('images',
                      osp.join('obs', obid)).replace('.png', '.txt').replace(
                          '.jpg', '.txt').replace('train',
                                                  'obs/COSTA_st/train')
        ...
        self.transforms = T.Compose(
            [T.ToTensor(), T.Normalize(opt.im_mean, opt.im_std)])
    ...
    def get_data(self, img_path, label_path):
        ...
        if os.path.isfile(label_path):
            ...
            labels = labels0.copy()
            labels[:,
                   0] = ratio * w * (labels0[:, 0] - labels0[:, 2] / 2) + padw
            labels[:,
                   1] = ratio * h * (labels0[:, 1] - labels0[:, 3] / 2) + padh
            labels[:,
                   2] = ratio * w * (labels0[:, 0] + labels0[:, 2] / 2) + padw
            labels[:,
                   3] = ratio * h * (labels0[:, 1] + labels0[:, 3] / 2) + padh
        else:
            labels = np.array([])
        nL = len(labels)
        if nL > 0:
...
class LoadImagesAndObsTAO:
    ...
    def __getitem__(self, index):
        ...
        if len(obs) == 0:
            ...
            cat_ids = np.array([x.get('category_id', -1) for x in obs])[:,
                                                                        None]
...
class LoadImagesAndMaskObsMOTS(LoadImagesAndObs):
    ...
    def get_data(self, img_path, label_path):
        ...
        img = np.ascontiguousarray(
            img[:, :, ::-1])  # BGR to RGB eg. (1080, 1920, 3)
        if self.transforms is not None:
            img = self.transforms(img)
...
class LoadImagesAndPoseObs(LoadImagesAndObs):
    def __init__(self, obs_jpath, opt):
        ...
        self.img_files = [
            osp.join(opt.data_root, p['image'][0]['name']) for p in self.infoj
        ]
        self.transforms = T.Compose(
            [T.ToTensor(), T.Normalize(opt.im_mean, opt.im_std)])
        self.use_lab = getattr(opt, 'use_lab', False)

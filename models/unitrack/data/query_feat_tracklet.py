...
class QueryFeatTube(object):
    ...
    def __repr__(self):
        return 'QFT_{}_({}_{})'.format(self.track_id, self.start_frame_id,
                                       self.end_frame_id)
    ...
    def update(self, query_feat, cur_frame_id):
        if self.end_frame_id < cur_frame_id:
            self.qf_tube.extend(
                [None for i in range(cur_frame_id - self.end_frame_id - 1)])
    ...
    def complete_empty_postfix(self, last_frame_idx):
        if len(self.qf_tube) == last_frame_idx + 1:
            return self
        self.qf_tube.extend(
            [None for i in range(last_frame_idx + 1 - self.end_frame_id)])

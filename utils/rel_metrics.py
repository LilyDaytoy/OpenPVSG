...
def calculate_final_metrics(relation_recall_dict, K_values):
    final_metrics = {}
    num_valid_rel = len([rel for rel in relation_recall_dict[K_values[0]].values() if rel['total'] != 0])
    for K in K_values:
        total_recall = sum(rel['hit'] for rel in relation_recall_dict[K].values())
        total_weak_recall = sum(rel['weak_hit'] for rel in relation_recall_dict[K].values())
        ...
        mean_recall = sum(rel['hit'] / rel['total'] for rel in relation_recall_dict[K].values() if rel['total'] != 0) / num_valid_rel
        weak_mean_recall = sum(rel['weak_hit'] / rel['total'] for rel in relation_recall_dict[K].values() if rel['total'] != 0) / num_valid_rel
        final_metrics[K] = {
            'recall': recall_at_k,
...
def calculate_viou(gt_set, pred_set, vid, data_dir):
    ...
    for range_pair in gt_span_list:
        ...
        for frame_id in range(start, end + 1):
            if frame_id >= len(pred_span_list):
                continue
            mask_path = os.path.join(masks_root, str(frame_id).zfill(4)+'.png')
            ...
            if frame_id in pred_sub_mask_dict and frame_id in pred_obj_mask_dict:
                # process subjects
                pred_sub_mask = pred_sub_mask_dict[frame_id].numpy().astype(bool)
                sub_iou = calculate_mask_iou(gt_sub_mask, pred_sub_mask)
                # process objects
                pred_obj_mask = pred_obj_mask_dict[frame_id].numpy().astype(bool)
    ...
    pred_hit_list_real = np.logical_and(pred_hit_list == 1, pred_span_list == 1).astype(pred_hit_list.dtype)

...
def viou_over_whole_length_video(tube1,
                                 durs1,
                                 tube2,
                                 durs2,
    ...
    tube1_has_relation = generate_helper_relation_indicator(
        tot_num_frames, durs1)
    tube2_has_relation = generate_helper_relation_indicator(
        tot_num_frames, durs2)

    # intersection for relation
    has_relation_overlap = np.logical_and(
        tube1_has_relation, tube2_has_relation
    )  # True indicates framse in the intersection of "having relation"
    tube1_has_relation_overlap = tube1[
        has_relation_overlap]  # the bboxes in the intersection of relation
    tube2_has_relation_overlap = tube2[
        has_relation_overlap]  # for bbox: [num_frames_intersection, 4]' mask: [num_frames_interset, w, h]

    # intersection for V
    if detection_method == 'bbox':
        v_intersection = compute_bboxes_intersection(
            tube1_has_relation_overlap, tube2_has_relation_overlap)
        # union for V
        tube1_v_union = compute_bboxes_union(tube1[tube1_has_relation])
        tube2_v_union = compute_bboxes_union(tube2[tube2_has_relation])
    else:
        v_intersection = compute_masks_intersection(
            tube1_has_relation_overlap, tube2_has_relation_overlap)
        # union for V
...
def eval_relations_of_singel_video(groundtruth,
    ...
    assert detection_method in [
        'bbox', 'pan_seg'
    ], 'Invalid detection method! Choose from [bbox, pan_seg]'
    if detection_method == 'pan_seg':
        ...
        gt_detected = np.zeros((len(gt_relations), ),
                               dtype=bool)  # if this gt relation is matched
        hit_scores = np.ones((len(pred_relations))) * -np.inf

        for pred_idx, pred_relation in enumerate(pred_relations):
            ov_max = -float('Inf')
            k_max = -1
            pred_triplet = pred_relation[
                'triplet']  # ((597, 54), (599, 21), 12)
            pred_cls_triplet = (pred_triplet[0][1], pred_triplet[1][1],
                                pred_triplet[2])  # (54, 21, 12)
            pred_s_id, pred_o_id = pred_triplet[0][0], pred_triplet[1][
                0]  # (597, 599, 12)
            ...
            for gt_idx, gt_relation in enumerate(gt_relations):
                ...
                gt_cls_triplet = (gt_triplet[0][1], gt_triplet[1][1],
                                  gt_triplet[2])
                if not gt_detected[
                        gt_idx] and pred_cls_triplet == gt_cls_triplet:
                    ...
                    subject_viou = viou_over_whole_length_video(
                        gt_subject_mask_tubes, gt_durs,
                        pred_subject_mask_tubes, pred_durs, detection_method)
                    object_viou = viou_over_whole_length_video(
                        gt_object_mask_tubes, gt_durs, pred_object_mask_tubes,
                        pred_durs, detection_method)
            ...
            if k_max >= 0:
                hit_scores[pred_idx] = pred_relation[
                    'score']  # finally, if no gt match, the score for this pred will remain -inf
...
def evaluate(groundtruths,
    ...
    tot_gt_relations = 0
    print('Computing average precision MAP over {} videos...'.format(
        len(groundtruths)))
    ...
    for groundtruth, prediction in zip(groundtruths, predictions):
        ...
        if len(groundtruth.relations) == 0:
            continue
        vid = groundtruth.vid
        tot_gt_relations += len(
            groundtruth.relations)  # for total number of gt_relation_intances
        vid_prec, vid_rec, vid_rel_scores = eval_relations_of_singel_video(
            groundtruth,
            prediction,
        ...
        for nre in topK_nreturns:
            cut_off = min(nre, vid_rel_scores.size
                          )  # det_scores.size -> number of pred_relations
    ...
    mean_ap = np.mean(list(
        video_ap.values()))  # mean_ap -> average on video level
    # calculate recall for detection
    rec_at_n = dict()
    for nre in topK_nreturns:
        scores = np.concatenate(
            tot_scores[nre])  # video level拆成relation_instances level
        tps = np.concatenate(
            tot_tp[nre])  # video level拆成relation_instances level
...
if __name__ == '__main__':
    with open(
            '/mnt/lustre/wxpeng/OpenPVSG/data/pvsg_v1_clean/val/val_video_names.json',
            'r') as f:
        vids = json.load(f)
    with open('/mnt/lustre/wxpeng/OpenPVSG/data/metric_input3/gt/data.pickle',
              'rb') as f:

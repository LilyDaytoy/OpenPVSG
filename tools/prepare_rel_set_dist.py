...
parser.add_argument('--data_dir', default='./data', help='path to pvsg dataset')
...
def process_video(vid):
    try:
        ...
        pred_feat_tubes = {query_feats[idx].track_id: query_feats[idx].qf_tube for idx in range(len(query_feats))}
...
with multiprocessing.Pool(processes=10) as pool:
    ...
    results = list(tqdm(pool.imap(process_video, video_list), total=len(video_list)))

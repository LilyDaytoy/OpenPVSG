...
def evaluate(subject_encoder, object_encoder, pair_proposal_model, relation_model, data_loader, num_top_pairs, relation_list, device, csv_file_path, mark):
    ...
    for i, relation_dict in enumerate(data_loader):
        with torch.no_grad():
            ...
            for object_dict in gt_dict['objects']:
                gt_object_dict[object_dict['object_id']] = object_dict['category']
            ...
            for gt_relation in gt_dict['relations']:
                ...
                for K_value in K_values:
                    ...
                    soft_relation_recall_dict[K_value][rel_key[2]]['total'] += 1
                ...
                for idx, result in enumerate(results):
                    ...
                    if (int(pred_obj_dict_with_mask[result['subject_index']]['cid'][0]),
                        int(pred_obj_dict_with_mask[result['object_index']]['cid'][0]),
                        ...
                        iou, iou_weak = calculate_viou(
                            ...
                            (pred_obj_dict_with_mask[result['subject_index']]['mask'],
                            pred_obj_dict_with_mask[result['object_index']]['mask'],
                            result['relation_span']),
                            vid, data_dir)

                        if weak_hit_flag == 0 and iou_weak >= 0.3:
                            weak_hit_flag = 1
                            for K_value in K_values:
                                if idx < K_value:
                        ...
                        if iou >= 0.3:
                            print(vid, relation_list[rel_key[2]], f"{100*iou:.2f}", f"{100*iou_weak:.2f}", flush=True)
                            for K_value in K_values:
                                if idx < K_value:
                                    relation_recall_dict[K_value][rel_key[2]]['hit'] += 1
                                    soft_relation_recall_dict[K_value][rel_key[2]]['hit'] += iou
                            break
    ...
    final_soft_metrics = calculate_final_metrics(soft_relation_recall_dict, K_values)
    for K in K_values:
        print('-------------------------------------------------------------------')
        ...
        print(f"Soft Mean Recall@{K}: {100 * final_soft_metrics[K]['mean_recall']:.2f}")
        print(f"Soft Weak Recall@{K}: {100 * final_soft_metrics[K]['weak_recall']:.2f}")
        print(f"Soft Weak Mean Recall@{K}: {100 * final_soft_metrics[K]['weak_mean_recall']:.2f}")
        print('-------------------------------------------------------------------')

...
def evaluate(subject_encoder, object_encoder, pair_proposal_model,
    ...
    for i, relation_dict in enumerate(data_loader):
        with torch.no_grad():
            ...
            # compute recall@20
            pair_recall = calculate_pair_recall_at_k(selected_pairs, gt_pairs,
                                                     20)
            ...
            for gt_relation in gt_relations:
                ...
                for idx, result in enumerate(results):
                    if (result['subject_index'], result['object_index'],
                            result['relation']) == rel_key:
                        # print("A successful recall for", relation_list[rel_key[2]], flush=True)
                        t_iou = calculate_iou(gt_relation['relation_span'],
...
if __name__ == '__main__':
    ...
    work_dir = f'./work_dirs/{split}_save_qf'
    save_work_dir = f'./work_dirs/relation/rel_ips_{model_name}_100e_pe_ln'
    ...
    pvsg_rel_dataset = PVSGRelationDataset(f'{data_dir}/pvsg.json', split,
                                           work_dir)
    ...
    pair_proposal_model.load_state_dict(
        loaded_state_dicts['pair_proposal_model'])
